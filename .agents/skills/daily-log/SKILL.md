---
name: daily-log
description: Scan all of today's Codex sessions plus git activity and write a short entry into the user's company daily journal file. Use when the user types /daily-log or asks for a work log, daily summary, standup note, or journal entry of what was done today.
---
...
1. **Today's prompts across every session:**
   ```bash
   python .Codex/skills/daily-log/scripts/scan_today.py
   ```
   Pass an explicit `YYYY-MM-DD` to log a different day. This reads
   `~/.Codex/projects/<slug>/*.jsonl` and filters on each entry's own `timestamp`
...
```bash
python .Codex/skills/daily-log/scripts/write_entry.py <<'EOF'
First point.
Second point.
EOF
```
