...
description: Review the work completed across today's Codex sessions and fold only the durable parts into AGENTS.md — architecture changes, conventions, commands, dependencies, constraints, and decisions. Use when the user types /update-Codex-md or asks to refresh, update, or top up AGENTS.md after a day's work.
---

# Update AGENTS.md

Fold today's work into `AGENTS.md` so the next session starts knowing what this one
learned. `AGENTS.md` is **standing guidance, not a changelog** — every line you add must
...
```bash
python .Codex/skills/update-Codex-md/scripts/scan_sessions.py   # every prompt, all sessions
...
a journal entry is a summary of activity, and what belongs in `AGENTS.md` is a different
...
line in `AGENTS.md` misleads every future session.

Then read the current `AGENTS.md` in full before editing. Much of what today's work
