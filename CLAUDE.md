...
**`TicketWorklog` is the single source of *billable* ticket time, and it is logged by hand.** Every screen that quotes hours reads that one aggregate: the Log time popover and Time spent column in the task grid, the ticket header chip, and the customer's support-hours pool (`customerProducts.productSupportHours` → `ticketHours`). It is deliberately an **on-read aggregate, not a stored counter** — deleting a worklog corrects itself. Only `MONTHLY` pools also get a stored write, into `SupportHoursLedger.used` via `adjustLoggedHours`.

- **There is no Time tab, and no ticket-level Log time button** — hours are booked from the task's own row (`TaskTimePopover`, the grid's Log time column), so time sits with the work it was spent on. Every UI gate that used to redirect elsewhere now sends the agent to Tasks. `WorklogSection` at the foot of the tab is read-and-delete only, and lists **just the orphans** — entries with no `taskId`, from before a task was the only way in; it hides itself once there are none.
...
- **The Tasks tab is a grid** — task · status · assignee · due · Log time · Time spent · comments — with a tick column. Clicking the row opens `TicketTaskDialog` (editable fields, description, derived time, status trail, comments); every in-cell control `stopPropagation`s or a quick status change also pops the dialog. Cell controls that must escape the table's `overflow-x-auto` use `components/ui/cell-popover.tsx` (`CellPopover`, portalled to `<body>`, flips above when short of room) — Radix `DropdownMenu` is wrong for these, its type-ahead swallows keystrokes and every panel here holds a text input.
- **Bulk actions run serially, never `Promise.all`.** Each delete reverses hours against the same month's `SupportHoursLedger` row, and concurrent writes race. The bulk Status control is offered only when the ticker qualifies on *every* selected row (mirrors `assertMayTrack`), and the selection is held as **ids**, so a refetch that reorders or drops a task cannot silently retarget it.
...
- **A task cannot move to `DONE` with nothing booked against it** (`assertTaskTimeLogged`: Σ `TicketWorklog` hours where `taskId` = this task). `CANCELLED` is deliberately exempt — work that turned out not to be needed is settled honestly with no time on it.
